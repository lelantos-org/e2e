...
## Why the price is unrealistic

The value is chosen for magnitude, not realism. A shielded fee is
...
fail on affordability rather than on what it checks.
...
Tests still derive the fee they expect rather than hardcoding it, because gas
moves between runs. See `feePaid` in `src/testkit/spend-fee.ts`.

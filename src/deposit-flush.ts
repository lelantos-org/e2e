...
// `DepositStream`.
//
// A deposit is escrowed on broadcast and enters the tree only when the
// relayer's flush tick drains it (`flush_interval_s` in config/relayer.toml).
// Polling the note cache conflates "the relayer has not flushed yet" with "it
 ...
 *
 * Advisory only, and never on the critical path: the stream opens once the
 * deposit is already broadcast, and a flush landing in that gap is missed.
 * Callers run this alongside the note-cache poll, which remains the source of
 * truth, and read the outcome only to explain a failure.
 */
export function watchDepositFlush(
    ...
    return {
        settled,
        // Formats against the timeout actually used, so a caller-supplied
        // value cannot drift from the message.

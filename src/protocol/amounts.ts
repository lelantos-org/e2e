// Amount units and the fee arithmetic between them.
//
...
// different magnitudes and are not interchangeable.
...
/** Circuit-unit amount literal, for the `CircuitAmount` the SDK wallet takes. */
...
// Mirrors the contract: fee = (publicIn * scale * feeBps) / 10000.
...
/**
 * Fee in circuit units, for amounts never scaled to base units — the debit a
 * withdraw takes off a shielded balance, for example. Mirrors the SDK's
 * `applyFee`. `feeFor` is the base-unit equivalent and scales first, so the two
 * floor at different magnitudes.
 */
 ...
 *
 * Three amounts: the principal, the pool's protocol fee, and the note paying
 * whoever flushes the deposit's second leaf. The relayer fee is additive, taken
 * from the payer's tokens rather than out of the deposit, so the depositor's
 * shielded balance is still `amount`.
 *
 * This is the number Permit2 signs over. Signing `withFee` instead reverts
 * inside Permit2 with `InvalidAmount(maxTotal)`, because the pool then asks for
 * more than the payer permitted.
 */
...
/**
 * Circuit units of slack minted on top of what a test spends, to cover relayer
 * fees.
 *
 * Funding happens in `beforeAll`, before the relayer can be asked what it
 * charges, and the charge moves with gas, so tests mint a margin rather than a
 * computed amount. Minting mock tokens is free; running short stalls a deposit
 * until its cancel delay.
 */

...
describe("edge: concurrent spends of one note", () => {
    ...
    it("only one of two parallel spends of the same note succeeds", async () => {
        ...
        // The coin selector picks the same input twice; the loser hits the
        // nullifier guard.
        ...
        // The decisive property, and why a rejection alone is not enough: one
        // note in, one credit out. bob is synced against the winner rather than
        // read from his still-empty local store, since an unsynced wallet
        // reporting zero would pass even if the note had been spent twice.
